...
#columns without ocean_proximity
...
with st.sidebar:
    ...
    model_type = st.selectbox(
        "Wybierz model",
        ["Regresja: Random Forest", "Klasyfikacja: XGBoost", "Gradient Boosting"]
    )
    n_estimators = st.sidebar.number_input("Liczba drzew", 50, 500, 100)
    max_depth = st.sidebar.slider("Maksymalna głębokość", 1, 20, 5)
    x_axis = st.selectbox("Oś X", numeric_columns, index=0)
    y_axis = st.selectbox("Oś Y", numeric_columns, index=3)
    color_by = st.selectbox("Koloruj według", numeric_columns)
...
if model_type == "Regresja":
    st.subheader("Wyniki Regresji")
    col1, col2 = st.columns(2)
    col1.metric("MAE", "0.45")
    col2.metric("R²", "0.85")
